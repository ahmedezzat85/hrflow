...
def backfill_all(client=None, domain: str = "all", dry_run: bool = False) -> dict:
    ...
    if hasattr(client, "is_backfilling"):
        client.is_backfilling = True
    ...
    try:
        with get_db_context() as db:
            ...
            if target_domain in ("all", "notes", "employee_notes"):
                ...
                for r in note_rows:
                    ...
                    note.note = str(r.get("note") or r.get("text") or "")
                    note.created_by = str(r.get("created_by") or r.get("author") or "")
            ...
            if target_domain in ("all", "documents", "employee_documents"):
                ...
                for r in doc_rows:
                    ...
                    doc.file_type = str(r.get("file_type") or r.get("mime_type") or "pdf")
                    doc.view_url = str(r.get("view_url") or r.get("drive_web_url") or "")
            ...
            if target_domain in ("all", "documents", "company_documents"):
                ...
                for r in cdoc_rows:
                    ...
                    if not cdoc:
                        ...
                        db.add(cdoc)
                    cdoc.name = str(r.get("name") or r.get("title") or "")
                    cdoc.category = str(r.get("category") or "General")
                    cdoc.drive_file_id = str(r.get("drive_file_id") or "")
                    cdoc.file_type = str(r.get("file_type") or r.get("mime_type") or "pdf")
                    cdoc.view_url = str(r.get("view_url") or r.get("drive_web_url") or "")
            ...
            if target_domain in ("all", "insurance", "insurance_categories"):
                ...
                for r in cat_rows:
                    ...
                    cat.annual_limit = _safe_float(r.get("annual_limit") or r.get("max_limit"))
        ...
        print("EXCEPTION IN BACKFILL:", exc)
        import traceback
        traceback.print_exc()
        raise
    finally:
        if hasattr(client, "is_backfilling"):
            client.is_backfilling = False
