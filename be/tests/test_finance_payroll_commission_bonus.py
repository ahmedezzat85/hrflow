...
def test_add_commission_and_bonus_lines(app_client, admin_cookies):
    ...
    initial_net = gen_resp.json()["total_net"]
    assert initial_net == 3000.0  # 2000 external + 1000 internal
...
def test_add_commission_to_employee_without_plan(app_client, admin_cookies):
    ...
    assert c_resp.json()["net_pay"] == 1500.0
    assert c_resp.json()["amount"] == 1500.0
...
def test_multiple_lines_same_type_same_employee(app_client, admin_cookies):
    ...
    assert {l["net_pay"] for l in sales_lines} == {400.0, 600.0}


def test_commissions_participate_in_statutory_obligations(app_client, admin_cookies):
    """Commissions participate in net totals; no statutory obligations auto-generated."""
    _cleanup()
    ...
    assert fin_data["total_net"] == 4500.0  # 3000 + 1000 + 500

    # Verify no StatutoryObligationDB records are auto-generated
    with get_db_context() as db:
...
def test_delete_draft_line_recalculates_totals(app_client, admin_cookies):
    ...
    assert run_after_del["total_net"] == 1000.0
